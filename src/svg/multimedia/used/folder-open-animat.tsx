...
export default function FolderOpenAnimate({drawerAnimate,setDetails}:eachSVGType){
...
export default function FolderOpenAnimate(){
    ...
    const animate = () =>{
        animationValue.value = withTiming({scale:1.1,translateY:-50,rotateZ:-2})
    }
    const inanimate = () =>{
    ...
    const path2AnimatedProps = useAnimatedProps(()=>{
        return{
            transform: [{translateY:animationValue.value.translateY}],
            rotation: animationValue.value.rotateZ
        }
    })
    return(
        <View 
